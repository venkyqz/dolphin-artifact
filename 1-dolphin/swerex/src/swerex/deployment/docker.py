...
__all__ = ["DockerDeployment", "DockerDeploymentConfig", "DockerStandbyDeployment", "DockerMountDeployment"]
...
class _BaseDockerDeployment(AbstractDeployment):
    def __init__(self, *, logger: logging.Logger | None = None, **kwargs):
        """
        Base deployment for docker.
        It SHOULD NOT be used directly.

        Args:
            **kwargs: Keyword arguments (see `DockerXXXConfig` for details).
        """
    ...
    async def stop(self):
        ...
        if self._runtime is not None:
            try:
                ...
                self.logger.warning(
                    f"Failed to close runtime for {self._container_name}; it may already be stopped: {e}",
                    exc_info=False,
                )
            finally:
                self._runtime = None
...
class DockerDeployment(_BaseDockerDeployment):
    def __init__(
        self,
        *,
...
class DockerMountDeployment(_BaseDockerDeployment):
    """
    Mount local directory to docker
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._config = DockerMountDeploymentConfig(**kwargs)

        # add -v local_path:remote_path when start container
        self._config.docker_args.append("-v")
        self._config.docker_args.append(
            f"{self._config.local_mount_path}:{self._config.remote_mount_path}"
        )

    @classmethod
    def from_config(cls, config: DockerMountDeploymentConfig) -> Self:
        return cls(**config.model_dump())


class DockerStandbyDeployment(_BaseDockerDeployment):
    """
    Use a standby docker image
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._config = DockerStandbyDeploymentConfig(**kwargs)
    ...
    @classmethod
    def from_config(cls, config: DockerStandbyDeploymentConfig) -> Self:
        return cls(**config.model_dump())
