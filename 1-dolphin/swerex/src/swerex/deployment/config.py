...
class WorkspaceDeploymentConfig(BaseModel):
    """Configuration for running local workspace deployment."""

    type: Literal["workspace"] = "workspace"

    workspace_root: str = Field(
        default="", description="The root path of the workspace."
    )

    model_config = ConfigDict(extra="forbid")

    def get_deployment(self) -> AbstractDeployment:
        from swerex.deployment.workspace import WrokspaceDeployment

        return WrokspaceDeployment.from_config(self)
...
class DockerMountDeploymentConfig(DockerDeploymentConfig):
    """Configuration for docker with local file mount."""

    local_mount_path: str | None = Field(default=None)
    """The path to the local directory to be mounted into the docker."""

    remote_mount_path: str = "/workspace"
    """The path to the remote directory to mount on."""

    type: Literal["mount"] = "mount"
    """Discriminator for (de)serialization/CLI. Do not change."""

    @model_validator(mode="after")
    def validate_and_transform_path(self) -> "DockerMountDeploymentConfig":
        if self.local_mount_path is not None:
            # we must convert it to absolute path for docker mount
            self.local_mount_path = Path(self.local_mount_path).absolute().__str__()
        return self
    ...
    def get_deployment(self) -> AbstractDeployment:
        ...
        return DockerMountDeployment.from_config(self)


class DockerStandbyDeploymentConfig(DockerDeploymentConfig):
    """Configuration to use a stand by docker container."""
    ...
    type: Literal["standby"] = "standby"
    """Discriminator for (de)serialization/CLI. Do not change."""

    @model_validator(mode="after")
    def validate_and_transform_path(self) -> "DockerStandbyDeploymentConfig":
        if self.local_mount_path is not None:
            # we must convert it to absolute path for docker mount
            self.local_mount_path = Path(self.local_mount_path).absolute().__str__()
        return self
    ...
    @override
    def get_deployment(self) -> AbstractDeployment:
        from swerex.deployment.docker import DockerStandbyDeployment

        return DockerStandbyDeployment.from_config(self)
