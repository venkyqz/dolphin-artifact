...
from sweagent.config.schema import (
    parse_swebench_uri,
)
...
class BasicCLI:
    ...
    def get_config(self, args: list[str] | None = None) -> BaseSettings:
        ...
        parser.add_argument(
            "--swebench",
            help="Swebench URI to config instance",
            required=False,
        )
        ...
        if cli_args.swebench:
            uri = cli_args.swebench
            swebench_config = parse_swebench_uri(uri)
            config = swebench_config.load_swebench_instance_config()
            config_merged["instances"] = config.model_dump()
