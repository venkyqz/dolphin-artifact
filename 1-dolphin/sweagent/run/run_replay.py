...
from sweagent.agent.agents import AbstractAgent, get_agent_from_config
...
class RunReplay:
    def __init__(
        ...
        *,
        ...
        self.agent_name_replay_path: dict = {}
        self._replay_action_trajs_path = tempfile.NamedTemporaryFile(suffix=".json").name
    ...
    def _get_config_from_agent(self, traj_data):
        replay_config: dict = {}
        try:
            # Handling main traj or sub-agent traj
            if isinstance(traj_data, list):
                traj_data = traj_data[0]
            replay_config = traj_data["replay_config"]
            config = RunSingleConfig.model_validate(replay_config)
        except KeyError:
        ...
        self.agent_name_replay_path[config.agent.name] = Path(tempfile.NamedTemporaryFile(suffix=".json").name)
        config.agent.model = ReplayModelConfig(replay_path=self.agent_name_replay_path[config.agent.name])
        if isinstance(config.agent, SOPAgentConfig):
            for ac in config.agent.agents:
                self.agent_name_replay_path[ac.name] = Path(tempfile.NamedTemporaryFile(suffix=".json").name)
                ac.model = ReplayModelConfig(replay_path=self.agent_name_replay_path[ac.name])
        return config
    ...
    def _create_actions_file(self) -> None:
        ...
        traj_data_list: list = []
        if isinstance(self._traj_data, list):
            traj_data_list = self._traj_data
        else:
            traj_data_list = [self._traj_data]

        for traj_data in traj_data_list:
            agent_name = traj_data["agent_name"]
            has_tool_calls = any(
            ...
            use_function_calling = parse_function in ["function_calling", "command_or_function_call"]
            ...
            if len(actions) == 0:
                ...
                raise ValueError(msg)
            if agent_name in self.agent_name_replay_path:
                with self.agent_name_replay_path[agent_name].open("a") as f:
                    f.write(json.dumps({self.instance_id: actions}))
                # self.agent_name_replay_path[agent_name].write_text(json.dumps({self.instance_id: actions}))

    def _get_env(self) -> SWEEnv:
        return SWEEnv.from_config(self.config.env)

    def _get_agent(self) -> AbstractAgent:
        # agent = DefaultAgent.from_config(self.config.agent)
        agent = get_agent_from_config(self.config.agent)
