...
from sweagent.agent.config import AgentConfig, DefaultAgentConfig, SOPAgentConfig
...
class RunBatchConfig(BaseSettings, cli_implicit_flags=False):
    ...
    agent: Optional[DefaultAgentConfig | SOPAgentConfig] = Field(
        default=None, description="Agent options."
    )
...
class RunBatch:
    ...
    def _run_instance(self, instance: BatchInstance) -> AgentRunResult:
        ...
        # We use name to load different agent classes, do not set it to problem id
