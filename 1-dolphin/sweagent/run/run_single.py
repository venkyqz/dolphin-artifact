...
from sweagent.run.run_filter import RunFilter
...
class RunSingleConfig(BaseSettings, cli_implicit_flags=False):
    instances: BatchInstanceSourceConfig | None = Field(
        default=None, description="Instances to run. For run single, only support to run the first."
    )
    env: EnvironmentConfig = Field(default_factory=EnvironmentConfig, description="Environment options.")
    agent: DefaultAgentConfig | SOPAgentConfig | None = Field(default=None, description="Agent options.")
    ...
    def model_post_init(self, context):
        """
        override post init for basemodel
        """
        super().model_post_init(context)
        if isinstance(self.problem_statement, EmptyProblemStatement) and self.instances:
            instances = self.instances.get_instance_configs()
            self.problem_statement = instances[0].problem_statement
...
class RunSingle:
    def __init__(
        ...
        *,
        ...
        config: RunSingleConfig | None = None,
    ):
        ...
        for hook in hooks or []:
            self.add_hook(hook)
        self.config: RunSingleConfig = config
    ...
    def from_config(cls, config: RunSingleConfig) -> Self:
        ...
        if isinstance(agent, IssueResolvingSOP):
            for ac in agent.agents:
                ac.replay_config = config
    ...
    def run(self):
        """
        the original run method in swe-agent
        """
        ...
        result = None
        if self.config and self.config.filter:
            run_filter = self.config.filter
            if run_filter.agent_name:
                agent = run_filter.filter_agent_by_name(self.agent)
                if agent:
                    result = agent.run(
        ...
        output_dirs = []
        output_dirs.append(self.output_dir / self.problem_statement.id)
        merge_predictions(output_dirs, self.output_dir / self.problem_statement.id / "preds.json")
