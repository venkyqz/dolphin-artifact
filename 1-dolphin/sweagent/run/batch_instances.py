    ...
    *,
    ...
    if instance_id_:
        instances = [
            ...
            if instance.problem_statement.id == instance_id_
        ]
        if not instances:
            raise ValueError(f"No instance found with id: {instance_id_}")
        return instances
...
class BaseInstances(BaseModel, AbstractInstanceSource):
    """Base class for common instance source fields."""
    ...
    """Directly set instance_id"""

    def filter_and_slice_instances(
        ...
        return _filter_batch_items(
            ...
            instance_id_=self.instance_id,
        )


class InstancesFromFile(BaseInstances):
    ...
    def get_instance_configs(self) -> list[BatchInstance]:
        ...
        return self.filter_and_slice_instances(instances)
...
class InstancesFromHuggingFace(BaseInstances):
...
class SWEBenchInstances(BaseInstances):
    ...
    _instances_cache: ClassVar[dict[str, list[BatchInstance]]] = {}
    ...
    def get_instance_configs(self) -> list[BatchInstance]:
        # Generate cache key based on dataset path and split
        cache_key = f"{self._get_dataset_path()}_{self.split}"
        ...
        if cache_key not in self._instances_cache:
            ...
            if isinstance(self.deployment, DockerDeploymentConfig):
                ...
                instances = [
                    SimpleBatchInstance.from_swe_bench(instance).to_full_batch_instance(self.deployment) for instance in
                    ds
                ]
                self._instances_cache[cache_key] = instances

        instances = self._instances_cache[cache_key]
        filtered_instances = self.filter_and_slice_instances(instances)
...
class ExpertInstancesFromFile(BaseInstances):
...
class SWESmithInstances(BaseInstances):
    ...
    def get_instance_configs(self) -> list[BatchInstance]:
        ...
        instance_dicts = load_file(self.path)
        instances = [
            SimpleBatchInstance.model_validate(
                convert_instance_dict(instance_dict)
            ).to_full_batch_instance(self.deployment)
            for instance_dict in instance_dicts
        ]
        return self.filter_and_slice_instances(instances)
