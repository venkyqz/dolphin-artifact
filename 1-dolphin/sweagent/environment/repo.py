...
class Repo(Protocol):
    ...
    def copy(self, deployment: AbstractDeployment, local_workspace: str = ""): ...
    ...
    def copy(self, deployment: AbstractDeployment, local_workspace: str = ""):
        ...
        asyncio.run(
            deployment.runtime.upload(
                UploadRequest(source_path=str(self.path), target_path=f"{local_workspace}/{self.repo_name}")
            )
        )
        r = asyncio.run(deployment.runtime.execute(Command(command=f"chown -R `whoami`:`id -g -n` {local_workspace}/{self.repo_name}", shell=True)))
        if r.exit_code != 0:
...
class GithubRepoConfig(BaseModel):
    ...
    def copy(self, deployment: AbstractDeployment, local_workspace: str = ""):
        ...
        path = f"{self.repo_name}" if local_workspace == "" else f"{local_workspace}/{self.repo_name}"
        asyncio.run(
