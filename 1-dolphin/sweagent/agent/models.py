...
from sweagent.agent.model_config import (
    deepseek_model_cost,
    glm_model_cost,
    kimi_model_cost,
    minimax_model_cost,
...
class GenericAPIModelConfig(PydanticBaseModel):
    ...
    type: str = Field(default="default", description="Type of the model client to use, default is LiteLLM.")
    cost_name: str = Field(description="Use another model to help calc cost for unsupported models", default="")
    provider: str = Field(
        default="",
        description="The client provider to use, set this if you want to use a compatible client, e.g. openai sdk.",
    )
    ...
    """If set, this will override the https proxy for the model that we usually look
    """

    # pydantic
    model_config = ConfigDict(extra="forbid")

    _named_class: ClassVar[dict[str, builtins.type[GenericAPIModelConfig]]] = {}

    def __init_subclass__(cls, **kwargs):
        if cls.name:
            if cls.name.default in cls._named_class:
                msg = "different model configs with same name"
                raise ValueError(msg)
            cls._named_class[cls.name.default] = cls
...
class AbstractModel(ABC):
    _named_class: dict[str, type[AbstractModel]] = {}
    name = ""

    def __init_subclass__(cls, **kwargs):
        # ignore empty
        if cls.name:
            if cls.name in cls._named_class:
                msg = "different model with same name"
                raise ValueError(msg)
            cls._named_class[cls.name] = cls
...
class ReplayModel(AbstractModel):
    ...
    def query(self, history: History) -> dict:
        ...
        if isinstance(action, str) and (action == "job_done" or action == "submit"):
            self._next_replay()
            return {"message": action}
...
class LiteLLMModel(AbstractModel):
    litellm.register_model(model_cost=qwen_model_cost)
    litellm.register_model(model_cost=kimi_model_cost)
    litellm.register_model(model_cost=glm_model_cost)
    litellm.register_model(model_cost=qwen3_model_cost)
    litellm.register_model(model_cost=qwen3_coder_model_cost)
    litellm.register_model(model_cost=qwen25_coder_model_cost)
    litellm.register_model(model_cost=openrouter_model_cost)
    litellm.register_model(model_cost=deepseek_model_cost)
    litellm.register_model(model_cost=minimax_model_cost)
    ...
    def __init__(self, args: GenericAPIModelConfig, tools: ToolConfig):
        ...
        # we may use the cost_name first (if given) for some unsupported models
        model_cost_config = litellm.model_cost.get(self.config.name, {}) or litellm.model_cost.get(
            self.config.cost_name, {}
        )

        # set max limit
        self.model_max_input_tokens = (
            self.config.max_input_tokens
            or model_cost_config.get("max_input_tokens")
            or model_cost_config.get("max_tokens")
        )
        self.model_max_output_tokens = (
            self.config.max_output_tokens
            or self.config.max_input_tokens
            or model_cost_config.get("max_output_tokens")
            or model_cost_config.get("max_tokens")
        )

        self.lm_provider = model_cost_config.get("litellm_provider")
    ...
    def _single_query(
        ...
        # if we use interact, we should disable this
        if self.tools.use_function_calling:
            extra_args["tools"] = self.tools.tools
            # set this arg could result in no THOUGHT and only ACTION inside the model response.
        ...
        if self.config.provider:
            extra_args["custom_llm_provider"] = self.config.provider
        ...
        try:
            if self.config.cost_name:
                response.model = self.config.cost_name
        ...
        # Use usage information from response instead of manually calculating
        if hasattr(response, "usage") and response.usage:
            input_tokens = response.usage.prompt_tokens
            output_tokens = response.usage.completion_tokens
        else:
            # Fallback to manual calculation if usage is not available
            input_tokens = litellm.utils.token_counter(messages=messages, model=self.config.name)
            # Calculate input tokens including function schemas if function calling is enabled
            if self.tools.use_function_calling:
                # Add function schemas to input tokens
                # NOTE: it is not accurate
                function_schemas = json.dumps([tool.to_openai_function().model_dump() for tool in self.tools.commands])
                input_tokens += litellm.utils.token_counter(
                    text=function_schemas, model=self.config.cost_name or self.config.name
                )
            ...
            for i in range(n_choices):
                ...
                if output:
                    output_tokens += litellm.utils.token_counter(
                        text=output, model=self.config.cost_name or self.config.name
                    )
                # Calculate tool calls tokens if function calling is enabled
                if self.tools.use_function_calling and response.choices[i].message.tool_calls:
                    for tool_call in response.choices[i].message.tool_calls:
                        # Add function name and arguments to output tokens
                        output_tokens += litellm.utils.token_counter(
                            text=json.dumps(tool_call.model_dump()), model=self.config.cost_name or self.config.name
                        )
...
def get_model(config: ModelConfig, tools: ToolConfig) -> AbstractModel:
    ...
    if isinstance(config, GenericAPIModelConfig):
        if config.name in GenericAPIModelConfig._named_class:
            config_cls = GenericAPIModelConfig._named_class[config.name]
            config = config_cls(**config.model_dump())

    if config.name in AbstractModel._named_class:
        model_cls = AbstractModel._named_class[config.name]
        return model_cls(config, tools)

    if config.type == "simple":
        from sweagent.agent.simplellm import SimpleLLMModel

        return SimpleLLMModel(config, tools)
    return LiteLLMModel(config, tools)

    # MENTION: keep original below for backup
    # if isinstance(config, GenericAPIModelConfig) and not isinstance(
    #     config, HumanModelConfig | HumanThoughtModelConfig | ReplayModelConfig | InstantEmptySubmitModelConfig
    # ):
    #     if config.name == "human":
    #         config = HumanModelConfig(**config.model_dump())
    #     elif config.name == "human_thought":
    #         config = HumanThoughtModelConfig(**config.model_dump())
    #     elif config.name == "replay":
    #         config = ReplayModelConfig(**config.model_dump())
    #     elif config.name == "instant_empty_submit":
    #         config = InstantEmptySubmitModelConfig(**config.model_dump())
    #
    # if config.name == "human":
    #     assert isinstance(config, HumanModelConfig), f"Expected {HumanModelConfig}, got {config}"
    #     return HumanModel(config, tools)
    # if config.name == "human_thought":
    #     assert isinstance(config, HumanThoughtModelConfig), f"Expected {HumanThoughtModelConfig}, got {config}"
    #     return HumanThoughtModel(config, tools)
    # if config.name == "replay":
    #     assert isinstance(config, ReplayModelConfig), f"Expected {ReplayModelConfig}, got {config}"
    #     return ReplayModel(config, tools)
    # elif config.name == "instant_empty_submit":
    #     assert isinstance(config, InstantEmptySubmitModelConfig), f"Expected {InstantEmptySubmitModelConfig}, got {config}"
    #     return InstantEmptySubmitTestModel(config, tools)
    # assert isinstance(config, GenericAPIModelConfig), f"Expected {GenericAPIModelConfig}, got {config}"
    # return LiteLLMModel(config, tools)
