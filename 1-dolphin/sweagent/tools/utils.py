...
def generate_command_docs(
    ...
    **kwargs,
    ...
    for cmd in commands + subroutine_types:
        # we define another special description
        if isinstance(cmd, Command):
            docs += cmd.format_as_cli_usage()
            docs += '\n'
            continue
        docs += f"{cmd.name}:\n"
...
def generate_command_tips(
    commands: list[Command],
    subroutine_types,
    **kwargs,
) -> str:
    """Generate concise command tips for quick reference.

    Format includes command name, brief description and basic usage.

    Args:
        commands: List of commands to generate tips for
        subroutine_types: List of subroutines to generate tips for
        **kwargs: Additional format variables for docstrings
    ...
    for cmd in commands + subroutine_types:
        if isinstance(cmd, Command):
            # For Command objects, use simplified format
            if cmd.tip:
                tips += f"\n## {cmd.name}\n{cmd.tip}\n"
            continue
...
def generate_command_json_schema(
    commands: list[Command],
    subroutine_types,
    **kwargs,
) -> str:
    ...
    for cmd in commands + subroutine_types:
        if isinstance(cmd, Command):
            tips += cmd.format_as_openai_tool()
        tips += "\n"
    return tips
