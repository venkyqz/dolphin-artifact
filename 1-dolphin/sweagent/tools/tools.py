...
from sweagent.tools import query
from sweagent.tools.bundle import Bundle, ShellBundle
...
from sweagent.tools.utils import _guard_multiline_input, generate_command_docs, generate_command_tips, \
    generate_command_json_schema
...
class ToolConfig(BaseModel):
    ...
    command_tips: str = None  # type: ignore
    command_json_schema: str = None  # type: ignore
    ...
    def use_function_calling(self) -> bool:
        return self.parse_function.use_function_call
            ...
            for command in bundle.commands:
                if command.name in tool_sources:
                    ...
                    msg = (
                        ...
                        f"  - Duplicate definition in: {bundle.position}"
                    )
                    raise ValueError(msg)
                commands.append(command)
                tool_sources[command.name] = bundle.position
    ...
    def model_post_init(self, __context):
        ...
        if not self.enable_bash_tool and not (
            any(
                [
                    ...
                    isinstance(self.parse_function, JsonParser),
                    isinstance(self.parse_function, BaseCommandParser),
                ]
            )
        ):
        ...
        self.command_tips = generate_command_tips(
            self.commands,
            [],
            **self.env_variables,
        )
        self.command_json_schema = generate_command_json_schema(
            self.commands,
            [],
            **self.env_variables,
        )
...
class ToolHandler:
    def __init__(self, tools: ToolConfig):
        ...
        self.config: ToolConfig = tools.model_copy(deep=True)
    ...
    def search_command(self, name) -> Optional[Command]:
        for c in self.config.commands:
            if c.name == name:
                return c
        return None

    def get_bundle(self, command: Command) -> Bundle | None:
    ...
    def get_agent(self, command: Command) -> "AbstractAgent":
        # Cache agent instances to avoid recreating them
        cache_key = f"{command.name}_{id(command)}"
        if cache_key in self.agent_cache:
            return self.agent_cache[cache_key]

        agent_bundle = self.get_bundle(command)
        agent_config = agent_bundle.config
    ...
    async def _check_available_commands(self, env: SWEEnv, env_vars: dict[str, str]) -> None:
        await asyncio.gather(
            *(self._is_command_available(env, command.name, env_vars) if command.type == "shell" else asyncio.sleep(0)
    ...
    def _install_commands(self, env: SWEEnv) -> None:
        ...
        # if we use local deployment, _upload_bundles still uploads to /root/tools, which is not good
        asyncio.run(self._upload_bundles(env))
        for bundle in self.config.bundles:

            # only install ShellBundle
            if not isinstance(bundle, ShellBundle):
                continue
    ...
    def invoke(self, call: ToolCallModel, env: SWEEnv = None, memory: Memory = None):
        resp = query.invoke(
            call,
            self.workspace,
            env=env,
